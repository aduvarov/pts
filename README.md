...
Изучение создания расширяемого приложения с плагинной архитектурой, использование архитектурных паттернов для упрощения добавления новых функций.
...
1.  Создание экзекьютора (исполнителя команд), например, для сжатия видео с помощью FFmpeg, с прикреплением логера.
2.  Демонстрация добавления новой команды: реализация другого экзекьютора для вывода содержимого текущей директории. Показано, что добавление новой команды не требует изменений в ядре приложения.
3.  Указание на необходимость преобразования типов данных для корректной работы (например, toString для буферов данных).
